...
function hideModal(modalElement) {
    ...
    // Очищаем форму, если это модальное окно транзакции
    if (modalElement === modal) {
        transactionForm.reset();
    }
    
    // Если это модальное окно категории или настроек, удаляем его полностью
    if (modalElement.classList.contains('category-modal') || modalElement.classList.contains('settings-modal')) {
        modalElement.remove();
    } else {
        modalElement.classList.remove('active');
    }
    
    // Очищаем обработчики событий модального окна
...
function generateIconOptions(selectedIcon) {
    const incomeIcons = [
        'fa-wallet',          // Кошелек
        'fa-money-bill',      // Деньги
        'fa-coins',           // Монеты
        'fa-piggy-bank',      // Копилка
        'fa-hand-holding-usd', // Деньги в руке
        'fa-gift',            // Подарок
        'fa-chart-line',      // График роста
        'fa-briefcase',       // Портфель
        'fa-building',        // Здание
        'fa-laptop',          // Ноутбук
        'fa-crown',           // Корона
        'fa-gem',             // Драгоценность
        'fa-award',           // Награда
        'fa-medal',           // Медаль
        'fa-trophy',          // Трофей
        'fa-university',      // Банк
        'fa-handshake',       // Рукопожатие
        'fa-percentage',      // Процент
        'fa-donate',          // Пожертвование
        'fa-credit-card'      // Кредитная карта
    ];

    const expenseIcons = [
        'fa-shopping-cart',    // Корзина покупок
        'fa-shopping-bag',     // Пакет покупок
        'fa-home',            // Дом
        'fa-car',             // Машина
        'fa-utensils',        // Столовые приборы
        'fa-hamburger',       // Еда
        'fa-pizza-slice',     // Пицца
        'fa-coffee',          // Кофе
        'fa-bolt',            // Электричество
        'fa-tshirt',          // Одежда
        'fa-shoe-prints',     // Обувь
        'fa-pills',           // Лекарства
        'fa-hospital',        // Больница
        'fa-graduation-cap',  // Образование
        'fa-book',            // Книги
        'fa-plane',           // Самолет
        'fa-train',           // Поезд
        'fa-bus',             // Автобус
        'fa-taxi',            // Такси
        'fa-bicycle',         // Велосипед
        'fa-gamepad',         // Игры
        'fa-film',            // Кино
        'fa-music',           // Музыка
        'fa-spa',             // Красота
        ...
        'fa-tint',            // Вода
        'fa-fire',            // Газ
        'fa-gift-card',       // Подарки
        'fa-theater-masks',   // Развлечения
        'fa-palette',         // Хобби
        'fa-ellipsis-h'       // Другое
    ];

    // Определяем, какой набор иконок использовать
    const icons = document.querySelector('form#categoryForm input[name="type"]')?.value === 'income' 
        ? incomeIcons 
        : expenseIcons;

    return icons.map(icon => `
        <option value="${icon}" ${icon === selectedIcon ? 'selected' : ''}>
            <i class="fas ${icon}"></i> ${icon.replace('fa-', '').split('-').join(' ')}
        </option>
    `).join('');
}
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            try {
                ...
                categories = data.categories && data.categories.income && data.categories.expense ? 
                ...
                // Обновляем язык последним, так как он вызовет обновление UI
                if (data.settings && data.settings.language) {
                    setLanguage(data.settings.language);
                } else {
                    // Если язык не указан, обновляем UI вручную
                    updateUI();
                }
...
function showTransactionModal(type, categoryId = null) {
    ...
    const modalHTML = `
        <div id="transactionModal" class="modal">
            <div class="modal-content">
                <h2>${translations[currentLang].addTransaction}</h2>
                <form id="transactionForm">
                    <div class="form-group">
                        <label>${translations[currentLang].type}</label>
                        <select name="type" required>
                            <option value="income" ${type === 'income' ? 'selected' : ''}>${translations[currentLang].income}</option>
                            <option value="expense" ${type === 'expense' ? 'selected' : ''}>${translations[currentLang].expense}</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>${translations[currentLang].amount}</label>
                        <input type="number" name="amount" step="0.01" required>
                    </div>
                    <div class="form-group">
                        <label>${translations[currentLang].category}</label>
                        <select name="category" required></select>
                    </div>
                    <div class="form-group">
                        <label>${translations[currentLang].date}</label>
    ...
    // Сначала обновляем список категорий для выбранного типа
    updateCategoryOptions(type);

    // Добавляем обработчик изменения типа транзакции
    ...
    modalElement.querySelector('.cancel-btn').addEventListener('click', () => hideModal(modalElement));
...
function updateCategoryOptions(type) {
    ...
    // Находим форму и select для категорий в текущем модальном окне
    const form = document.getElementById('transactionForm');
    if (!form) {
...
function handleTransactionSubmit(e) {
    ...
    if (isNaN(amount) || amount <= 0) {
        alert(translations[currentLang].warning + ': ' + translations[currentLang].amount + ' ' + translations[currentLang].notFound);
        return;
    }
    ...
    try {
        ...
        alert(translations[currentLang].warning + ': ' + error.message);
    ...
    categories.income.forEach(category => {
        ...
        categoryCard.innerHTML = `
            <div class="category-left">
                ...
                <div class="category-info">
                    <span class="category-name">${category.name[currentLang]}</span>
                </div>
            </div>
            <div class="category-actions">
                <button class="edit-category" title="${translations[currentLang].editCategory}">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="delete-category" title="${translations[currentLang].deleteCategory}">
                    <i class="fas fa-trash"></i>
                </button>
            </div>
        `;
        
        // Добавляем быстрое добавление транзакции по клику на всю карточку категории
        categoryCard.addEventListener('click', (e) => {
            // Проверяем, не был ли клик по кнопкам действий
        ...
        categoryCard.querySelector('.edit-category').addEventListener('click', (e) => {
            e.stopPropagation();
...
// Обновляем обработчик для кнопок закрытия
