...
const createElementByType = (type, titleParam, content, index, elementData = {}) => {
  ...
  switch (type.toLowerCase()) {
    ...
    case 'accordion':
      let accordionItems = [];
      console.log('[createElementByType] Processing accordion with content:', cleanContent);
      if (cleanContent) {
    ...
    case 'faq-section':
      ...
      if (cleanContent) {
        ...
        pairs.forEach((pair, index) => {
          // Разделяем вопрос и ответ по вертикальной черте
    ...
    case 'advanced-contact-form': {
      ...
      if (cleanContent) {
        ...
        if (parts.length >= 20) {
          formTitle = parts[0];
          fieldLabels.name = parts[1];
          fieldLabels.email = parts[2];
          fieldLabels.phone = parts[3];
          fieldLabels.company = parts[4];
          fieldLabels.subject = parts[5];
          fieldLabels.message = parts[6];
          fieldLabels.submitButton = parts[7];
          fieldLabels.submittingButton = parts[8];
          fieldLabels.newsletter = parts[9];
          fieldLabels.terms = parts[10];
          placeholders.name = parts[11];
          placeholders.email = parts[12];
          placeholders.phone = parts[13];
          placeholders.company = parts[14];
          placeholders.message = parts[15];
          subjectOptions[0].label = parts[16];
          subjectOptions[1].label = parts[17];
          subjectOptions[2].label = parts[18];
          subjectOptions[3].label = parts[19];
          ...
          fieldLabels.submittingButton = parts[8];
          fieldLabels.newsletter = parts[9];
          if (parts[10]) fieldLabels.terms = parts[10];
        } else if (parts.length >= 4) {
          formTitle = parts[0];
          fieldLabels.name = parts[1];
          fieldLabels.email = parts[2];
          fieldLabels.phone = parts[3];
          if (parts[4]) fieldLabels.company = parts[4];
          if (parts[5]) fieldLabels.subject = parts[5];
          if (parts[6]) fieldLabels.message = parts[6];
          if (parts[7]) fieldLabels.submitButton = parts[7];
        } else if (parts.length >= 1) {
          formTitle = parts[0];
        }
      }
