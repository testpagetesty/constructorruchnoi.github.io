...
const EditorPanel = ({
  ...
  const handleCreateUAEPaymentsSite = () => {
    ...
    // Создаем расширенные пункты меню
    const newMenuItems = [
  ...
  const generateContentElementHTML = (element) => {
    ...
    switch (element.type) {
      case 'gradient-text':
        return `
          <div id="${elementId}" class="content-element gradient-text" style="
            margin: 1rem 0;
            text-align: center;
          ">
            <h2 style="
              background: linear-gradient(${element.direction || 'to right'}, ${element.color1 || '#ff6b6b'}, ${element.color2 || '#4ecdc4'});
            ...
            ">${element.text || 'Градиентный текст'}</h2>
          </div>
        `;
        ...
        return `
          ...
          ">
            <h3 style="
              ...
              margin-bottom: 1rem;
              font-size: 1.5rem;
            ">${element.title || 'Статистика'}</h3>
            <div style="
              font-size: 3rem;
              font-weight: bold;
              color: ${element.countColor || '#1976d2'};
              margin-bottom: 0.5rem;
            ">
              <span class="counter" 
                    data-start="${element.startValue || 0}" 
                    data-end="${element.endValue || 100}"
                    data-duration="${element.duration || 2000}">
      ...
      case 'typewriter-text':
        const texts = Array.isArray(element.texts) ? element.texts : ['Привет, мир!', 'Добро пожаловать', 'На наш сайт'];
        return `
          <div id="${elementId}" class="content-element typewriter-text" style="
            margin: 2rem 0;
            text-align: center;
            min-height: 60px;
            display: flex;
          ...
          ">
            <h2 class="typewriter" 
                ...
                data-speed="${element.speed || 150}"
                data-pause="${element.pauseTime || 2000}"
                data-repeat="${element.repeat !== false}"
                style="
        ...
        return `
          ...
          ">
            <p style="
              ...
              color: ${element.textColor || '#333333'};
              background: ${element.highlightColor || '#ffeb3b'};
              padding: 0.5rem 1rem;
              border-radius: 8px;
              display: inline-block;
              margin: 0;
            ">${element.text || 'Это важный текст с выделением'}</p>
          </div>
        `;

      case 'testimonial-card':
        const stars = '★'.repeat(Math.floor(element.rating || 5)) + '☆'.repeat(5 - Math.floor(element.rating || 5));
        return `
          ...
          ">
            <div style="display: flex; align-items: center; margin-bottom: 1rem;">
              ${element.avatar ? `
                <img src="${element.avatar}" alt="${element.name}" style="
                  width: 60px;
                  height: 60px;
                  border-radius: 50%;
                  margin-right: 1rem;
              ...
              ` : `
                <div style="
                  ...
                  margin-right: 1rem;
                ">
                  ${(element.name || 'И').charAt(0).toUpperCase()}
                </div>
              `}
              <div>
                <h4 style="margin: 0; color: #333; font-size: 1.2rem;">${element.name || 'Иван Иванов'}</h4>
                <p style="margin: 0; color: #666; font-size: 0.9rem;">${element.role || 'Генеральный директор'}</p>
                <p style="margin: 0; color: #888; font-size: 0.8rem;">${element.company || 'ООО "Компания"'}</p>
              </div>
            </div>
            <div style="color: #ffc107; margin-bottom: 1rem; font-size: 1.2rem;">
              ${stars}
            </div>
            <p style="
              color: #555;
              ...
              line-height: 1.6;
              margin: 0;
            ">"${element.content || 'Отличный сервис! Рекомендую всем.'}"</p>
          </div>
        `;
      ...
      case 'typography':
        ...
        return `
          <div id="${elementId}" class="content-element typography" style="
            margin: 1rem 0;
      ...
      case 'rich-text':
        ...
        return `
          <div id="${elementId}" class="content-element rich-text" style="
            margin: 1.5rem 0;
            padding: 1rem;
            background: #fafafa;
            border-radius: 8px;
          ...
          ">
            ${element.showTitle !== false ? `
              <h3 style="
                color: ${element.titleColor || '#1976d2'};
                margin: 0 0 1rem 0;
                font-size: 1.25rem;
              ">${element.title || 'Богатый текст'}</h3>
            ` : ''}
            <div style="
              color: ${element.textColor || '#333333'};
              line-height: 1.6;
              font-size: 1rem;
            ">
              <p>${parseMarkdown(element.content || 'Текст с **жирным**, *курсивом*, ***жирным курсивом***\n\nВторой абзац с [ссылкой](https://example.com)')}</p>
            </div>
          </div>
        `;

      case 'code-block':
        const codeLines = (element.code || 'function hello() {\n  console.log("Hello, World!");\n}').split('\n');
        ...
        return `
          ...
          ">
            ${element.showTitle !== false ? `
              ...
              ">
                <span>📄</span>
                ${element.title || 'Блок кода'} (${element.language || 'javascript'})
              </div>
            ` : ''}
            <div style="
              background: #2d2d2d;
      ...
      case 'callout':
        const calloutTypes = {
          ...
          error: { bg: '#ffebee', border: '#d32f2f', icon: '❌' },
          success: { bg: '#e8f5e8', border: '#388e3c', icon: '✅' },
          note: { bg: '#f3e5f5', border: '#7b1fa2', icon: '📝' },
          tip: { bg: '#e8f8f5', border: '#00796b', icon: '💡' }
        };
        const calloutType = calloutTypes[element.calloutType] || calloutTypes.info;
        
        return `
          <div id="${elementId}" class="content-element callout" style="
            margin: 1.5rem 0;
            padding: 1rem 1.5rem;
            background: ${calloutType.bg};
            border: 1px solid ${calloutType.border};
            border-radius: 8px;
            border-left: 4px solid ${calloutType.border};
          ...
          ">
            <div style="display: flex; align-items: flex-start; gap: 0.75rem;">
              ...
              <div style="flex: 1;">
                <h4 style="
                  margin: 0 0 0.5rem 0;
                  color: ${element.titleColor || calloutType.border};
                  font-size: 1.1rem;
                  font-weight: 600;
                ">${element.title || 'Важная информация'}</h4>
                <p style="
                  margin: 0;
                  color: ${element.contentColor || '#333333'};
                  line-height: 1.5;
                  font-size: 0.95rem;
                ">${element.content || 'Это важная информация, которую пользователи должны заметить.'}</p>
              </div>
            </div>
          </div>
        `;
      ...
      case 'blockquote':
        return `
          <div id="${elementId}" class="content-element blockquote" style="
            margin: 1.5rem 0;
            padding: 1rem 1.5rem;
          ...
          ">
            <blockquote style="margin: 0; font-style: italic;">
              <p style="
                color: ${element.quoteColor || '#555555'};
                font-size: 1.1rem;
                line-height: 1.6;
                margin: 0 0 1rem 0;
              ">"${element.quote || 'Важная цитата или высказывание'}"</p>
              <footer style="
                color: ${element.authorColor || '#888888'};
                font-size: 0.9rem;
                text-align: right;
              ">
                — ${element.author || 'Автор цитаты'}${element.source ? ', ' + element.source : ''}
              </footer>
            </blockquote>
          </div>
        `;
      ...
      case 'list':
        const listTag = element.listType || 'ul';
        const items = element.items || ['Первый элемент', 'Второй элемент', 'Третий элемент'];
        return `
          <div id="${elementId}" class="content-element list" style="
            margin: 1.5rem 0;
            padding: 0 1rem;
          ">
            <${listTag} style="
      ...
      case 'basic-card':
        return `
          ...
          ">
            ...
            <p style="
              color: ${element.contentColor || '#666666'};
              line-height: 1.5;
              margin: 0;
  ...
  const generateMultiPageSection = (siteData, sectionId, sectionData) => {
    ...
    if (hasImages) {
      if (sectionData.images.length === 1) {
        ...
        imagesHtml = `
          <div class="section-gallery" style="
            width: 100%;
            margin: 2rem auto;
            text-align: center;
            max-width: 600px;
...
<body>
    ...
    <main>
        ...
        <section class="section-content" style="
          padding: 3rem 0;
          background: ${sectionData.showBackground !== false ? bgColor : 'transparent'};
          border-radius: 20px;
          margin: 2rem auto;
          max-width: 1200px;
          box-shadow: ${sectionData.showBackground !== false ? '0 10px 30px rgba(0,0,0,0.08)' : 'none'};
          position: relative;
          overflow: hidden;
        ">
  ...
  const handleDownloadSite = async () => {
    try {
      ...
      if (currentConstructorMode) {
        ...
        console.log('🏗️ Generating single page site (Constructor mode)');
        const htmlContent = generateHTML(siteData);
        zip.file('index.html', htmlContent);
      } else {
        // Ручной режим - генерируем многостраничный сайт
        console.log('📄 Generating multi-page site (Manual mode)');
        ...
        Object.entries(sectionsData).forEach(([sectionId, sectionData]) => {
          const fileName = getSectionFileNameSafe(sectionId, sectionData);
          if (fileName) {
            const pageContent = generateMultiPageSection(siteData, sectionId, sectionData);
