...
const ElementEditor = ({ element, onElementChange, onElementDelete, onElementDuplicate, isSelected = false }) => {
  ...
  const renderTestimonialEditor = () => (
    <Stack spacing={3}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 2 }}>
        <Box>
          <Typography variant="body2" gutterBottom>Цвет имени:</Typography>
          <TextField
            fullWidth
            type="color"
            value={element.nameColor || '#1976d2'}
        ...
        <Box>
          <Typography variant="body2" gutterBottom>Цвет должности:</Typography>
          <TextField
            fullWidth
            type="color"
            value={element.roleColor || '#666666'}
            onChange={(e) => handleChange('roleColor', e.target.value)}
            size="small"
          />
        </Box>
        
        <Box>
          <Typography variant="body2" gutterBottom>Цвет компании:</Typography>
          <TextField
            fullWidth
            type="color"
            value={element.companyColor || '#888888'}
        ...
        <Box>
          <Typography variant="body2" gutterBottom>Цвет текста отзыва:</Typography>
          <TextField
            fullWidth
            type="color"
  ...
  const renderImageGalleryEditor = () => (
    <Stack spacing={3}>
      <Alert severity="info">
        <Typography variant="body2">
          💡 <strong>Совет:</strong> Дважды кликните на элемент галереи в превью для полноценного редактирования с загрузкой изображений и настройками отображения
        </Typography>
      </Alert>
    ...
    <Stack spacing={2}>
      ...
      <Box sx={{ display: 'flex', gap: 2 }}>
        ...
        <TextField
          label="Цвет текста"
          type="color"
          value={element.contentColor || '#666666'}
          onChange={(e) => handleChange('contentColor', e.target.value)}
          sx={{ width: 120 }}
        />
      </Box>
