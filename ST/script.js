...
function preventIOSBounce() {
    // Переменные для отслеживания начала касания
    let startX = 0;
    let startY = 0;
    ...
    // Фиксируем только боковой скролл всего документа, но разрешаем вертикальный скролл в контейнерах
    document.addEventListener('touchmove', function(e) {
        // Разрешаем скролл в контейнерах, которые должны прокручиваться
        const isScrollableElement = e.target.closest('.transactions-container, .categories-grid, .modal-content, .reports-content, input, select, textarea');
        ...
        // Если мы не в скроллируемом контейнере и это горизонтальный скролл, то предотвращаем его
        if (!isScrollableElement && Math.abs(deltaX) > Math.abs(deltaY)) {
            e.preventDefault();
        }
    }, { passive: false });
