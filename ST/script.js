...
function handleTouchStart(e) {
    ...
    // Only prevent default if we're in a draggable context, not in the transaction modal
...
function handleTouchMove(e) {
    const categoryItem = e.target.closest('.category-item');
    if (categoryItem) {
        // Mark that this touch has moved (not a tap)
        categoryItem.touchMoved = true;
    }
    
    // Only prevent default and handle drag if not in the transaction modal
    if (!categoryItem || !categoryItem.closest('.transaction-type-modal')) {
        e.preventDefault();
...
function handleTouchEnd(e) {
    ...
    // If we're in the transaction modal and this was a tap (not a drag)
    if (categoryItem && categoryItem.closest('.transaction-type-modal')) {
        const touchEndTime = new Date().getTime();
        const touchDuration = touchEndTime - (categoryItem.touchStartTime || 0);
        
        // If it was a short touch (tap) and didn't move much
        if (touchDuration < 300 && !categoryItem.touchMoved) {
