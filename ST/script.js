...
const state = {
    ...
    touchMovedForCategory: false // Добавляем это поле
};
...
function initializeDragAndDrop() {
    ...
    // Загружаем сохраненные позиции категорий
    loadCategoryPositions();
...
function handleTouchStart(event) {
    // Сбрасываем флаг движения касания
    state.touchMovedForCategory = false;
    
    const touch = event.changedTouches[0];
    const categoryItem = touch.target.closest('.category-item');
    
    // Если касание не на элементе категории, выходим
    if (!categoryItem) return;
    
    // Проверяем, находится ли категория в модальном окне транзакций
    const isInTransactionModal = categoryItem.closest('.transaction-type-modal');
    
    // Записываем время начала касания для определения долгого нажатия
    state.dragState.lastTouchTimestamp = Date.now();
    
    // Если категория не в модальном окне (т.е. на главном экране), делаем ее перетаскиваемой
    if (!isInTransactionModal) {
        categoryItem.isDraggable = true;
        startDrag(categoryItem, touch.clientX, touch.clientY);
    }
}

function handleTouchMove(event) {
    // Помечаем, что касание перемещается (не просто тап)
    state.touchMovedForCategory = true;
    
    const touch = event.changedTouches[0];
    state.dragState.currentX = touch.clientX;
    state.dragState.currentY = touch.clientY;
    
    // Если не в режиме перетаскивания, выходим
    if (!state.dragState.isDragging) return;
    
    // Чтобы не прокручивать страницу во время перетаскивания
    event.preventDefault();
    
    // Обновляем позицию клона элемента
    if (state.dragState.dragClone) {
        state.dragState.dragClone.style.left = `${touch.clientX - state.dragState.offsetX}px`;
        state.dragState.dragClone.style.top = `${touch.clientY - state.dragState.offsetY}px`;
        
        // Находим элемент под текущей позицией пальца
        ...
        // Если под пальцем находится категория, подсвечиваем ее
        if (elemBelow) {
            const categoryBelow = elemBelow.closest('.category-item');
...
function handleTouchEnd(event) {
    ...
    // Если нет элемента категории, просто выходим
    if (!categoryItem) {
        // Всё равно завершаем перетаскивание, если оно было активно
    ...
    // Проверяем контекст - в модальном окне или на главном экране
    const isInTransactionModal = categoryItem.closest('.transaction-type-modal');
    
    // Если было перетаскивание, завершаем его
    if (state.dragState.isDragging && state.dragState.dragElement === categoryItem) {
        endDrag();
        return;
    }
    
    // Если это короткое нажатие (тап) и не было движения
    if (!state.touchMovedForCategory) {
        // Очищаем любые таймеры долгого нажатия
        if (state.dragState.longPressTimer) {
            clearTimeout(state.dragState.longPressTimer);
            state.dragState.longPressTimer = null;
        }
        
        // Для категорий в модальном окне - обрабатываем выбор категории
        if (isInTransactionModal) {
        ...
        // Для категорий на главном экране - открываем модальное окно
        else {
            const categoryType = categoryItem.dataset.type;
            showTransactionTypeModal(categoryType);
        }
    }
    
    // Сбрасываем состояние перетаскивания для данного элемента
    categoryItem.isDraggable = false;
}
...
function startDrag(element, x, y) {
    ...
    // Устанавливаем таймер длительного нажатия (500мс)
    state.dragState.longPressTimer = setTimeout(() => {
        // Отмечаем, что начато перетаскивание
        state.dragState.isDragging = true;
        state.dragState.dragElement = element;
        
        // Добавляем анимацию тряски к оригинальному элементу
        ...
        // Вычисляем размер и позицию клона
        ...
        // Сохраняем смещение для корректного позиционирования при перетаскивании
...
function endDrag() {
    ...
    // Если не в режиме перетаскивания, просто выходим
    if (!state.dragState.isDragging) return;
    
    const draggedElement = state.dragState.dragElement;
    
    // Выполняем остальную логику только если есть элемент перетаскивания
    if (draggedElement) {
        // Проверяем, куда перетаскиваем (элемент под курсором/пальцем)
        ...
        // Если есть целевая категория и она отличается от перетаскиваемой
        if (dropCategory && dropCategory !== draggedElement) {
            ...
            // Вибрация для обратной связи при успешном перемещении
            if (window.navigator && window.navigator.vibrate) {
    ...
    // Удаляем клон элемента, если он существует
    if (state.dragState.dragClone) {
...
// Функция для отображения модального окна выбора типа транзакции
function showTransactionTypeModal(type = 'expense') {
    // Открываем модальное окно
    const transactionTypeModal = document.querySelector('.transaction-type-modal');
    if (transactionTypeModal) {
        transactionTypeModal.classList.add('active');
        
        // Устанавливаем активный тип
        document.querySelectorAll('.type-button').forEach(btn => btn.classList.remove('active'));
        document.querySelector(`.type-button[data-type="${type}"]`).classList.add('active');
        
        // Показываем соответствующую сетку категорий
        document.querySelectorAll('.categories-grid').forEach(grid => grid.classList.remove('active'));
